...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  // Modal Pratinjau & Cetak Dokumen Kartu Peminatan (PDF)
  const [selectedRespondentPdf, setSelectedRespondentPdf] = useState<RespondentRecord | null>(null);
  ...
  return (
    <div className="space-y-6">
      {/* KONTEN PORTAL ADMINISTRATOR (DISEMBUNYIKAN SAAT MENCETAK KARTU PDF) */}
      <div className={selectedRespondentPdf ? 'print:hidden space-y-6' : 'space-y-6'}>
        {/* HEADER PORTAL ADMIN */}
                      ...
                      <td className="p-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-blue-950 hover:bg-blue-900 text-white font-bold text-xs transition cursor-pointer shadow-2xs"
                          ...
                          <button
                            type="button"
                            onClick={() => setSelectedRespondentPdf(r)}
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs transition cursor-pointer shadow-2xs"
                            title="Lihat & Cetak Dokumen Kartu Peminatan (PDF)"
                          >
      ...
      {selectedMinatModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in duration-200 print:hidden">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl bg-blue-950 hover:bg-blue-900 text-white text-xs font-bold transition flex items-center gap-1 cursor-pointer shadow-2xs"
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                setSelectedRespondentPdf(r);
                              }}
                              className="px-2.5 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition flex items-center gap-1 cursor-pointer shadow-2xs"
                              title="Lihat & Cetak Dokumen Kartu Peminatan (PDF)"
                            >
          ...
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-xs sm:text-sm">
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 flex-wrap shrink-0">
                  <button
                    type="button"
                    onClick={() => setSelectedRespondentPdf(selectedRespondentDetail)}
                    className="px-3.5 py-2.5 rounded-xl bg-blue-950 hover:bg-blue-900 text-white font-bold text-xs shadow-xs transition flex items-center gap-2 cursor-pointer border border-amber-400/50"
                    title="Buka Pratinjau & Cetak Kartu Peminatan (PDF)"
                  >
                    <Printer className="w-4 h-4 text-amber-300" />
                    <span>Lihat & Cetak Kartu PDF</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => downloadSingleSurveyExcel(selectedRespondentDetail.data, selectedRespondentDetail.score)}
                    className="px-3.5 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs shadow-xs transition flex items-center gap-2 cursor-pointer"
                  >
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
              <button
                type="button"
                onClick={() => setSelectedRespondentPdf(selectedRespondentDetail)}
                className="px-4 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs transition cursor-pointer flex items-center gap-2 shadow-2xs"
                title="Buka Pratinjau & Cetak Dokumen Kartu Peminatan (PDF)"
              >
                <Printer className="w-4 h-4" />
                <span>Buka Kartu Peminatan (PDF)</span>
              </button>
              <button
                type="button"
                onClick={() => setSelectedRespondentDetail(null)}
      ...
      {selectedRespondentPdf && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-950/80 overflow-y-auto print:static print:p-0 print:m-0 print:bg-transparent print:backdrop-blur-none print:z-auto print:overflow-visible">
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-300 overflow-hidden flex flex-col max-h-[92vh] print:max-h-none print:h-auto print:overflow-visible print:border-none print:shadow-none print:rounded-none print:w-full print:p-0">
            ...
            <div className="p-4 sm:p-5 bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 text-white flex items-center justify-between gap-3 shrink-0 print:hidden border-b-2 border-amber-400">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base sm:text-lg font-black text-white">
                      Pratinjau Kartu Peminatan Pra-Pensiun (PDF)
                    </h3>
                    <span className="hidden sm:inline-block px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-400/30">
                      Dokumen Resmi ASN
                    </span>
                  </div>
                  <p className="text-xs text-blue-200 mt-0.5">
                    Responden: <strong className="text-white">{selectedRespondentPdf.data.nama}</strong> {selectedRespondentPdf.data.nip && `(NIP: ${selectedRespondentPdf.data.nip})`}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-4 py-2 sm:py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs sm:text-sm shadow-md transition flex items-center gap-2 cursor-pointer"
                >
                  <Printer className="w-4 h-4" />
                ...
                <button
                  type="button"
                  onClick={() => setSelectedRespondentPdf(null)}
            ...
            {/* Dokumen Body (Scrollable di layar, full print di cetak) */}
            <div className="p-3 sm:p-6 md:p-8 overflow-y-auto flex-1 bg-slate-100/70 print:bg-white print:p-0 print:m-0 print:overflow-visible">
              <ResultCard
                result={selectedRespondentPdf.score}
                data={selectedRespondentPdf.data}
                isAdminPreview={true}
                onClose={() => setSelectedRespondentPdf(null)}
              />
            </div>
          </div>
        </div>
